...
func TestSignKnownVector(t *testing.T) {
	// Independently recompute and verify our `sign()` matches.
	...
	// The expected output is base64 of hex (which is 64 chars), so the result
	// is exactly 88 characters (no padding for 64-byte input).
	if len(got) != 88 {
		t.Errorf("sign length=%d want 88", len(got))
	}
}

// pairServer simulates a Philips TV serving /6/pair/request and /6/pair/grant.
// expectedPIN: if non-empty, the grant endpoint returns INVALID_PIN unless the
...
type pairServer struct {
	...
	requestCount int32
	grantCount   int32
	digestReqs   int32 // number of grant requests that included Authorization
}
...
func (p *pairServer) handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/6/pair/grant":
			...
			// First request: 401 with digest challenge.
...
func newPairTestClient(t *testing.T, srv *httptest.Server) *Client {
	t.Helper()
...
func TestPairFlow_HappyPath(t *testing.T) {
	...
	// grant: one un-auth'd + one auth'd retry
