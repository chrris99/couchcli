...
type Frame = philips.AmbilightColors
...
type Effect interface {
	Frame(topo philips.AmbilightTopology, t time.Duration) Frame
}

// Driver is the minimal slice of *philips.AmbilightService the runner
// needs. *philips.AmbilightService satisfies this interface
// structurally, so production callers pass c.Ambilight; tests pass an
// in-memory fake.
type Driver interface {
	Topology(ctx context.Context) (philips.AmbilightTopology, error)
	SetMode(ctx context.Context, m philips.AmbilightMode) error
	SetCached(ctx context.Context, c philips.AmbilightColors) error
}
...
func Run(ctx context.Context, d Driver, e Effect, fps int, dur time.Duration, restore bool) error {
	...
	if err := d.SetMode(ctx, philips.AmbilightModeManual); err != nil {
	...
	if restore {
		defer func() {
			...
			defer cancel()
			_ = d.SetMode(restoreCtx, philips.AmbilightModeInternal)
		}()
	}
	...
	for {
		...
		if err := d.SetCached(ctx, e.Frame(topo, elapsed)); err != nil {
...
func scaleColor(c philips.AmbilightColor, intensity float64) philips.AmbilightColor {
...
func totalLEDs(topo philips.AmbilightTopology) int {
...
func edgeForIndex(topo philips.AmbilightTopology, perimeterIdx int) (edge string, idx int) {
...
func buildFrame(topo philips.AmbilightTopology, colorAt func(perimeterIdx int) philips.AmbilightColor) Frame {
	total := totalLEDs(topo)
	if total == 0 {
		return Frame{"layer1": philips.AmbilightLayer{}}
	}

	makeEdge := func(count, offset int) map[string]philips.AmbilightColor {
		if count <= 0 {
			return nil
		}
		m := make(map[string]philips.AmbilightColor, count)
		for i := 0; i < count; i++ {
			m[strconv.Itoa(i)] = colorAt(offset + i)
		}
		return m
	}

	layer := philips.AmbilightLayer{
...
func hsvToRGB(h, s, v float64) philips.AmbilightColor {
