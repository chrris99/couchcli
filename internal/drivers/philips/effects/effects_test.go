...
var topo3sided = jointspace.AmbilightTopology{Layers: 1, Top: 8, Right: 3, Bottom: 0, Left: 3}

// Hypothetical 4-sided device.
var topo4sided = jointspace.AmbilightTopology{Layers: 1, Top: 8, Right: 4, Bottom: 8, Left: 4}
...
func TestHelpers_BuildFrame_OmitsEmptyEdge(t *testing.T) {
	f := buildFrame(topo3sided, func(int) Color {
		return Color{R: 1}
	})
...
func TestWave_FrameShape(t *testing.T) {
	w := Wave{Color: Color{G: 255}, Period: 2 * time.Second}
...
func TestPulse_AllSameColor(t *testing.T) {
	p := Pulse{Color: Color{R: 200}, Period: time.Second, Floor: 0.1}
	...
	for _, m := range []map[string]jointspace.AmbilightColor{f["layer1"].Top, f["layer1"].Right, f["layer1"].Bottom, f["layer1"].Left} {
		for k, c := range m {
			if c != want {
...
func TestComet_HeadAndTail(t *testing.T) {
	c := Comet{Color: Color{R: 200, G: 100}, Period: time.Second, TailLen: 4}
...
func TestFire_StableShapeAndChannels(t *testing.T) {
	...
	any := false
	for _, m := range []map[string]jointspace.AmbilightColor{frame["layer1"].Top, frame["layer1"].Right, frame["layer1"].Left} {
		for _, c := range m {
...
func TestBuildFrame_ZeroLEDs(t *testing.T) {
	f := buildFrame(jointspace.AmbilightTopology{Layers: 1}, func(int) Color {
		return Color{}
	})
	if f == nil {
		t.Fatal("nil frame")
...
func TestWave_IntensityRange(t *testing.T) {
	w := Wave{Color: Color{G: 255}, Period: time.Second}
	min, max := 256, -1
	for i := 0; i < 100; i++ {
