...
type fakeDriver struct {
	mu        sync.Mutex
	calls     []string
	topology  jointspace.AmbilightTopology
	frames    []Frame
	topoErr   error
	modeErr   error
	cachedErr error
...
func (f *fakeDriver) Topology(_ context.Context) (jointspace.AmbilightTopology, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeDriver) SetMode(_ context.Context, m jointspace.AmbilightMode) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeDriver) SetCachedColors(_ context.Context, c jointspace.AmbilightColors) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func TestRun_SequenceAndRestore(t *testing.T) {
	d := &fakeDriver{topology: jointspace.AmbilightTopology{Layers: 1, Top: 4, Right: 2, Bottom: 0, Left: 2}}
	eff := Wave{Color: Color{R: 0, G: 255, B: 0}, Period: 500 * time.Millisecond}
...
func TestRun_NoRestore(t *testing.T) {
	d := &fakeDriver{topology: jointspace.AmbilightTopology{Layers: 1, Top: 2}}
	eff := Wave{Color: Color{R: 255}, Period: time.Second}
...
func TestRun_EmptyTopology(t *testing.T) {
	d := &fakeDriver{topology: jointspace.AmbilightTopology{Layers: 1}}
